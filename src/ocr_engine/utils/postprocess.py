...
class DBPostProcess:
    ...
    def __call__(self, pred, shape_list):
        ...
        for batch_index in range(pred.shape[0]):
            ...
            boxes = self.boxes_from_bitmap(pred[batch_index], mask, src_w, src_h)
...
class CTCLabelDecode:
    def __init__(self, character_dict_path):
        ...
        with open(character_dict_path, "rb") as fin:
            for line in fin.readlines():
                self.character.append(line.decode("utf-8").strip("\n").strip("\r\n"))
    ...
    def decode(self, text_index, text_prob, is_remove_duplicate=True):
        ...
        for batch_idx in range(batch_size):
            ...
            selection &= text_index[batch_idx] != 0 # ignore blank (idx 0)
            
            char_list = [self.character[int(t_id)] for t_id in text_index[batch_idx][selection]]
            conf_list = text_prob[batch_idx][selection] if len(text_prob[batch_idx][selection]) > 0 else [0]
            text = "".join(char_list)
